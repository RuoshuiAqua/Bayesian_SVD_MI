...
with open(f"./output/prior/prior_distribution_parameters_{missing_type}_m_{m}_n_{n}_K_{K}.pkl", 'rb') as file_handle:
...
for idx_cycle in range(num_cycles):
    ...
    if idx_cycle >= num_cycles//2:
        ...
        Y_est = Y_est + np.matmul(Uh*Dh, Vh.T)
...
os.makedirs(f"./output/record", exist_ok = True)
with open(f"./output/record/samples_missing_{missing_type}_{m}_{n}_{K_true}.pkl", 'wb') as file_handle:
