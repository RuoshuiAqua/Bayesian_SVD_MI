...
## missing flag
flag_missing = np.loadtxt(f"./output/omega/omega_{missing_type}_m_{m}_n_{n}_K_{K}_beta_{beta}.csv", delimiter = ",")
flag_missing = flag_missing.astype(np.bool)

with open(f"./output/prior/prior_distribution_parameters_{missing_type}_m_{m}_n_{n}_K_{K}_beta_{beta}.pkl", 'rb') as file_handle:
...
for idx_cycle in range(num_cycles):
    print(f"idx of cycle: {idx_cycle}, lambda_{lamb:.4f}, phi: {phi:.4f}", flush = True)
    ...
    phi_record.append(np.copy(phi))        
    tau_square_record.append(np.copy(tau_square))
    ...
    if idx_cycle >= num_cycles//2 and idx_cycle % 2 == 0:
...
with open(f"./output/record/samples_missing_{missing_type}_{m}_{n}_{K_true}_beta_{beta}.pkl", 'wb') as file_handle:
    pickle.dump({'Uh': np.array(Uh_record), 'Vh': np.array(Vh_record), 'Dh': np.array(Dh_record),
                 'lamb': np.array(lamb_record), 'Yh': np.array(Yh_record), 'phi': np.array(phi_record),
                 'tau_square': np.array(tau_square_record), 'Y_est': Y_est}, file_handle)
